...
class OTPlanSampler:
    ...
    def get_map(self, x0, x1):
        ...
        if not np.all(np.isfinite(p)):  # type: ignore
            print("ERROR: p is not finite")
            print(p)
            print("Cost mean, max", M.mean(), M.max())
            print(x0, x1)
        if np.abs(p.sum()) < 1e-8:  # type: ignore
            if self.warn:
                warnings.warn("Numerical errors in OT plan, reverting to uniform plan.")
            p = np.ones_like(p) / p.size  # type: ignore
        return p
...
class ExactOptimalTransportConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def guided_sample_location_and_conditional_flow(
        self, x0, x1, y0=None, y1=None, t=None, return_noise=False, replace=True
    ):
        r"""
        Compute the sample xt (drawn from N(t * x1 + (1 - t) * x0, sigma))
        and the conditional vector field ut(x1|x0) = x1 - x0, see Eq.(15) [1]
        ...
        [1] Improving and Generalizing Flow-Based Generative Models with minibatch optimal transport, Preprint, Tong et al.
        """
        x0, x1, y0, y1 = self.ot_sampler.sample_plan_with_labels(
            x0, x1, y0, y1, replace=replace
        )
        if return_noise:
            t, xt, ut, eps = super().sample_location_and_conditional_flow(  # type: ignore
                x0, x1, t, return_noise
            )
            return t, xt, ut, y0, y1, eps
        else:
            t, xt, ut = super().sample_location_and_conditional_flow(x0, x1, t, return_noise)  # type: ignore
...
class VariancePreservingConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def compute_mu_t(self, x0, x1, t):
        ...
        """
        t = pad_t_like_x(t, x0)
        return torch.cos(math.pi / 2 * t) * x0 + torch.sin(math.pi / 2 * t) * x1  # type: ignore
    ...
    def compute_conditional_flow(self, x0, x1, t, xt):
        ...
        t = pad_t_like_x(t, x0)
        return (
            math.pi / 2 * (torch.cos(math.pi / 2 * t) * x1 - torch.sin(math.pi / 2 * t) * x0)  # type: ignore
        )
